...
<template>
  ...
  <FormKit type="form" submit-label="Poslat" #default="{ value }" v-model="formValue" @submit="submitHandler"
    ...
    <section class="simple-section">
      ...
      <!-- Jmeno firmy -->
      <FormKit type="text" label="Jméno firmy" label-class="text-secondary" name="jmeno_firmy" placeholder=""
        help="Přidej pouze firmu, která zatím není v databázi." validation="required|length:2,100" :validation-messages="{
      ...
      <FormKit type="url" label="E-shop firmy (zkopírujte)" label-class="text-secondary" name="eshop" placeholder=""
        help="Člověk musí mít možnost si od firmy něco koupit (B2C)." validation="required|url" :validation-messages="{
    ...
    <!-- Pridavani vyroben +- -->
    <FormKit v-model="list_vyroben" type="list" name="vyrobny">
      <Vyrobna v-for="item in aktualni_vyrobny" :key="item" :factory_number="item"
        :removable="aktualni_vyrobny.length > 1" @remove="remove_from_list_by_id(aktualni_vyrobny, $event, 1)" />
    </FormKit>
    <div class="row">
